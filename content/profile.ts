/**
 * MJ's CV.
 *
 * Transcribed from the CV spread of `assets/press/landscape-portfolio-2026.pdf`,
 * the press file for his current book, which MJ supplied himself and named as
 * the reference. That supersedes the 2025 Behance transcription and the older
 * graphic-book CV in CONTENT-INVENTORY.md section D.
 */

export const bio = [
  'I have a background in architecture and have gained experience working in various architecture firms. Alongside that I have freelanced in graphic design. Throughout my career I have tried to embody the qualities of an architect, an artist, and a meticulous engineer.',
  'My involvement in environmental conservation is what pushed me toward a master’s in landscape architecture. I wanted to deepen my understanding of environmental protection and design rather than treat it as a specialism someone else handles.',
  'In practice that means I do not hand a project over between disciplines. The site analysis, the masterplan, the diagrams that explain it and the identity that presents it are made in the same place, by the same person, which is why they tend to agree with each other.',
]

export const education = [
  {
    degree: 'M.Arch in Landscape Architecture, Land Landscape Heritage',
    school: 'Politecnico di Milano',
    place: 'Italy',
    dates: 'Sep 2022 – Oct 2025',
  },
  {
    degree: 'B.Sc in Architecture Engineering',
    school: 'Pars University of Art and Architecture',
    place: 'Tehran, Iran',
    dates: 'Sep 2015 – Sep 2019',
  },
]

export const experience = [
  {
    role: 'Interior Designer',
    org: 'GN Architetti',
    place: 'Milan, Italy',
    dates: 'Jan 2026 – Feb 2026',
    work: 'Project based · Drawing assistant · Architectural presentation · Interior design',
  },
  {
    role: 'Architect',
    org: 'Cognition Architecture Studio',
    place: 'Milan, Italy, remote',
    dates: 'Nov 2025 – Jan 2026',
    work: 'Drawing assistant · Landscape design · Architectural presentation',
  },
  {
    role: 'Junior Architect',
    org: 'Ar.de.a. Arturo Montanelli Studio Design',
    place: 'Lecco, Italy',
    dates: 'Sep 2024 – Feb 2025',
    work: 'Drawing assistant · Landscape design · Interior design',
  },
  {
    role: 'Architect and Landscape Architect',
    org: 'Four and Seven',
    place: 'Iran',
    dates: 'Dec 2021 – 2025',
    work: 'Urban playground design · Landscape design · Interior design',
  },
  {
    role: 'Architect and Graphic Designer',
    org: 'Karoshan Construction Company',
    place: 'Iran',
    dates: 'Oct 2019 – Dec 2021',
    work: 'Playground design · Interior design · Brand book · Logo · Catalogue',
  },
  {
    role: 'Junior Architect (apprenticeship)',
    org: 'Hajizadeh & Associates',
    place: 'Iran',
    dates: 'Jan 2020',
    work: 'Architectural diagrams · Maquettes',
  },
  {
    role: 'Junior Architect',
    org: 'Vavan Urban Planning and Construction Company',
    place: 'Iran',
    dates: 'Jan 2019 – Jun 2019',
    work: 'Construction design · Interior design · Landscape design',
  },
  {
    role: 'Junior Architect',
    org: 'Houshang Ardalan and Partners Architecture Office',
    place: 'Iran',
    dates: 'Nov 2017 – Oct 2018',
    work: 'Architectural design · Landscape design · Interior design',
  },
  {
    role: 'Graphic and Architecture Manager',
    org: 'Golabchi Foundation',
    place: 'Iran',
    dates: 'Jul 2017 – Nov 2017',
    work: 'Design management · Logo · Catalogue · Social media graphics',
  },
]

export const workshops = [
  { title: '21st Century Architecture Kaleidoscope seminar', date: 'Oct 2018' },
  { title: 'Architecture of Illusion: fantasy in architecture and its psychological impact', date: 'Sep 2020' },
  { title: 'Dialogues on Milan: City as a Common Resource, with a lecture by Alejandro Aravena', date: 'Apr 2024' },
  { title: 'Anatomy of Public Space: public space and the human body', date: 'May 2024' },
]

export const tools = [
  'Rhinoceros 3D',
  'Grasshopper',
  'AutoCAD',
  'Revit',
  'AllPlan',
  'SketchUp',
  'QGIS',
  'Twinmotion',
  'Lumion',
  'V-Ray',
  'Illustrator',
  'Photoshop',
  'InDesign',
  'Figma',
  'Hand drawing',
]

export const languages = [
  { name: 'Persian', level: 'Native' },
  { name: 'English', level: 'Fluent' },
  { name: 'Italian', level: 'Elementary' },
]

export const interests = ['Sketching', 'Architecture photography', 'Theatre', 'Literature']
